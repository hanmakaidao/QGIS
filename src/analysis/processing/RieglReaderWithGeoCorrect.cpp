/******************************************************************************
* Copyright (c) wangpeng @art  wangpeng187@126.com
*
****************************************************************************/

#define _USE_MATH_DEFINES
#include "qgsalgorithmsbetpointcloud.h"
#include <cmath>
#include <cstring>
#include <sstream>

#include <pdal/PDALUtils.hpp>
#include <pdal/SpatialReference.hpp>
#include <pdal/StageFactory.hpp>

static StaticPluginInfo const s_info
{
    "readers.riegl",
    "riegl reader support.",
    "http://pdal.io/stages/readers.riegl.html",
    { "Riegl" }
};

CREATE_STATIC_STAGE(RieglReaderWithGeoCorrect, s_info)

std::string RieglReaderWithGeoCorrect::getName() const
{
    return s_info.name;
}

#ifndef _WIN32
const size_t RieglReaderWithGeoCorrect::MaximumNumberOfReturns;
const size_t RieglReaderWithGeoCorrect::MaxNumRecordsInBuffer;
const size_t RieglReaderWithGeoCorrect::NumBytesInRecord;
#endif

RieglReaderWithGeoCorrect::RieglReaderWithGeoCorrect()
    : Reader()
    , m_header()
    , m_boresightMatrix(georeference::createIdentityMatrix())
    , m_istream()
    , m_buffer()
    , m_extractor(m_buffer.data(), 0)
    , m_recordIndex(0)
    , m_returnIndex(0)
    , m_pulse()
{}


const RieglHeader& RieglReaderWithGeoCorrect::getHeader() const { return m_header; }


void RieglReaderWithGeoCorrect::initialize()
{
    std::istream* rawStream = Utils::openFile(m_filename);
    if (!rawStream)
        throwError("Unable to open " + m_filename + " for reading.");
    ILeStream stream(rawStream);

    try
    {
        stream.get(m_header.signature, 4);
        if (strcmp(m_header.signature, "Riegl") != 0)
            throwError("Invalid header signature when reading Riegl file: '" +
                std::string(m_header.signature) + "'");

        stream.get(m_header.vendorId, 64);
        stream.get(m_header.softwareVersion, 32);
        stream >> m_header.formatVersion >> m_header.headerSize >>
            m_header.gpsWeek >> m_header.minTime >> m_header.maxTime >>
            m_header.numRecords >> m_header.numStrips;
        for (size_t i = 0; i < 256; ++i)
        {
            stream >> m_header.stripPointers[i];
        }
        stream >> m_header.misalignmentAngles[0] >>
            m_header.misalignmentAngles[1] >> m_header.misalignmentAngles[2] >>
            m_header.imuOffsets[0] >> m_header.imuOffsets[1] >>
            m_header.imuOffsets[2] >> m_header.temperature >> m_header.pressure;
        stream.get(m_header.freeSpace, 830);
    }
    catch( ... )
    {
        Utils::closeFile(rawStream);
        throw;
    }
    Utils::closeFile(rawStream);

    m_boresightMatrix = createRieglRotationMatrix(
        m_header.misalignmentAngles[0] + m_header.imuOffsets[0],
        m_header.misalignmentAngles[1] + m_header.imuOffsets[1],
        m_header.misalignmentAngles[2] + m_header.imuOffsets[2]);

    // The riegl docs say that their lat/longs are referenced
    // to the WGS84 reference frame.
    setSpatialReference("EPSG:4326");
}


void RieglReaderWithGeoCorrect::addDimensions(PointLayoutPtr layout)
{
    using namespace Dimension;

    layout->registerDims( { Id::X, Id::Y, Id::Z, Id::GpsTime, Id::ReturnNumber,
        Id::NumberOfReturns, Id::EchoRange, Id::Intensity,
        Id::ScanAngleRank } );
}


void RieglReaderWithGeoCorrect::ready(PointTableRef)
{
    m_istream.reset(new IStream(m_filename));
    if (!*m_istream)
        throwError("Unable to open " + m_filename + " for reading.");

    m_istream->seek(m_header.headerSize);
    m_recordIndex = 0;
    m_returnIndex = 0;
    m_pulse = RieglPulse();
}


point_count_t RieglReaderWithGeoCorrect::read(PointViewPtr data, point_count_t countRequested)
{
    point_count_t numRead = 0;
    point_count_t dataIndex = data->size();

    while (numRead < countRequested)
    {
        if (m_returnIndex == 0)
        {
            if (!m_extractor.good())
            {
                if (m_recordIndex >= m_header.numRecords)
                {
                    break;
                }
                m_recordIndex += fillBuffer();
            }

            m_extractor >> m_pulse.gpsTime >> m_pulse.returnCount >>
                m_pulse.range[0] >> m_pulse.range[1] >> m_pulse.range[2] >>
                m_pulse.range[3] >> m_pulse.intensity[0] >>
                m_pulse.intensity[1] >> m_pulse.intensity[2] >>
                m_pulse.intensity[3] >> m_pulse.scanAngle >> m_pulse.roll >>
                m_pulse.pitch >> m_pulse.heading >> m_pulse.latitude >>
                m_pulse.longitude >> m_pulse.elevation;

            if (m_pulse.returnCount == 0)
            {
                m_returnIndex = 0;
                continue;
            }

            // In all the Riegl files that we've tested, the longitude
            // values have been less than -2pi.
            if (m_pulse.longitude < -M_PI * 2)
            {
                m_pulse.longitude = m_pulse.longitude + M_PI * 2;
            }
            else if (m_pulse.longitude > M_PI * 2)
            {
                m_pulse.longitude = m_pulse.longitude - M_PI * 2;
            }
        }

        georeference::Xyz gpsPoint = georeference::Xyz(m_pulse.longitude, m_pulse.latitude, m_pulse.elevation);
        georeference::RotationMatrix rotationMatrix =createRieglRotationMatrix(m_pulse.roll, m_pulse.pitch,m_pulse.heading);
        georeference::Xyz point = pdal::georeference::georeferenceWgs84(m_pulse.range[m_returnIndex], m_pulse.scanAngle,m_boresightMatrix, rotationMatrix, gpsPoint);

        data->setField(Dimension::Id::X, dataIndex, point.X * 180 / M_PI);
        data->setField(Dimension::Id::Y, dataIndex, point.Y * 180 / M_PI);
        data->setField(Dimension::Id::Z, dataIndex, point.Z);
        data->setField(Dimension::Id::GpsTime, dataIndex, m_pulse.gpsTime);
        if (m_returnIndex == MaximumNumberOfReturns - 1)
        {
            data->setField(Dimension::Id::ReturnNumber, dataIndex,m_pulse.returnCount);
        }
        else
        {
            data->setField(Dimension::Id::ReturnNumber, dataIndex,
                          m_returnIndex + 1);
        }
        data->setField(Dimension::Id::NumberOfReturns, dataIndex,m_pulse.returnCount);
        data->setField(Dimension::Id::EchoRange, dataIndex,m_pulse.range[m_returnIndex]);
        data->setField(Dimension::Id::Intensity, dataIndex,m_pulse.intensity[m_returnIndex]);
        data->setField(Dimension::Id::ScanAngleRank, dataIndex,m_pulse.scanAngle * 180 / M_PI);

        if (m_cb)
            m_cb(*data, dataIndex);

        ++dataIndex;
        ++numRead;
        ++m_returnIndex;

        if (m_returnIndex >= m_pulse.returnCount ||
            m_returnIndex >= MaximumNumberOfReturns)
        {
            m_returnIndex = 0;
        }
    }
    return numRead;
}


size_t RieglReaderWithGeoCorrect::fillBuffer()
{
    size_t numRecords = (std::min)(m_header.numRecords - m_recordIndex,
        MaxNumRecordsInBuffer);

    buffer_size_t bufferSize = NumBytesInRecord * numRecords;
    m_buffer.resize(bufferSize);
    m_istream->get(m_buffer);
    m_extractor = LeExtractor(m_buffer.data(), m_buffer.size());
    return numRecords;
}


void RieglReaderWithGeoCorrect::done(PointTableRef)
{
    m_istream.reset();
}


