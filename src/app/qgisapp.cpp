...
=======
  ...
  */
static void setTitleBarText_(QWidget &qgisApp)
>>>>>>> 4df3fc0ad4210fdbd20c76e2d5a7fa08cfa041e0
{
  QString caption;
  ...
  caption += QgisApp::tr("JHL—ART—RockAll");
...
=======
  ...
  mGeometryValidationService->setMessageBar(mInfoBar);
  mGeometryValidationDock = new QgsGeometryValidationDock(tr("Geometry Validation"), mMapCanvas, this);
>>>>>>> 8d6357eb213466b34fef120c6a88f6b1aea127d3
...
void QgisApp::ImportReconstruction(const std::string& path) {
...
void QgisApp::CreateActions() {
  ...
  connect(action_project_save_as_, &QAction::triggered, this,
    &QgisApp::ProjectSaveAs);

  action_import_ =
    new QAction(QIcon(":/media/import.png"), tr("Import model"), this);
  connect(action_import_, &QAction::triggered, this, &QgisApp::Import);
  ...
  connect(action_export_, &QAction::triggered, this, &QgisApp::Export);
  ...
  connect(action_export_as_, &QAction::triggered, this, &QgisApp::ExportAs);
  ...
  connect(action_automatic_reconstruction_, &QAction::triggered, this,
    &QgisApp::AutomaticReconstruction);
  ...
  connect(action_reconstruction_reset_, &QAction::triggered, this,
    &QgisApp::ReconstructionOverwrite);
  ...
  connect(action_reconstruction_normalize_, &QAction::triggered, this,
    &QgisApp::ReconstructionNormalize);
  ...
  connect(action_render_options_, &QAction::triggered, this,
    &QgisApp::RenderOptions);

  connect(
    reconstruction_manager_widget_,
    static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),
    this, &QgisApp::SelectReconstructionIdx);
  ...
  connect(action_log_show_, &QAction::triggered, this, &QgisApp::ShowLog);
  ...
  connect(action_render_, &QAction::triggered, this, &QgisApp::Render,
    Qt::BlockingQueuedConnection);

  action_render_now_ = new QAction(tr("Render now"), this);
  render_options_widget_->action_render_now = action_render_now_;
  connect(action_render_now_, &QAction::triggered, this, &QgisApp::RenderNow,
    Qt::BlockingQueuedConnection);
  ...
  connect(action_reconstruction_finish_, &QAction::triggered, this,
    &QgisApp::ReconstructionFinish, Qt::BlockingQueuedConnection);

  action_about_ = new QAction(tr("About"), this);
  connect(action_about_, &QAction::triggered, this, &QgisApp::About);
  action_documentation_ = new QAction(tr("Documentation"), this);
  connect(action_documentation_, &QAction::triggered, this,
    &QgisApp::Documentation);
  action_support_ = new QAction(tr("Support"), this);
  connect(action_support_, &QAction::triggered, this, &QgisApp::Support);
...
void QgisApp::CreateStatusbar() {
  ...
  connect(statusbar_timer_, &QTimer::timeout, this, &QgisApp::UpdateTimer);
...
void QgisApp::CreateControllers() {
  if (mapper_controller_) {
    mapper_controller_->Stop();
    mapper_controller_->Wait();
  }
...
void QgisApp::ProjectEdit() {
  project_widget_->show();
  project_widget_->raise();
}

void QgisApp::ProjectSave() {
...
void QgisApp::FeatureExtraction() {
  if (options_.Check()) {
...
void QgisApp::DatabaseManagement() {
  if (options_.Check()) {
...
void QgisApp::ReconstructionStep() {
  if (mapper_controller_->IsFinished() && HasSelectedReconstruction()) {
    QMessageBox::critical(this, "",
      tr("Reset reconstruction before starting."));
    return;
...
void QgisApp::ReconstructionPause() {
  timer_.Pause();
...
void QgisApp::ReconstructionReset() {
  CreateControllers();
...
void QgisApp::ReconstructionNormalize() {
  if (!IsSelectedReconstructionValid()) {
    return;
  }
  action_reconstruction_step_->setEnabled(false);
...
bool QgisApp::ReconstructionOverwrite() {
  if (reconstruction_manager_.Size() == 0) {
    ReconstructionReset();
    return true;
  }
...
void QgisApp::DenseReconstruction() {
...
void QgisApp::RenderNow() {
  reconstruction_manager_widget_->Update();
  RenderSelectedReconstruction();
}

void QgisApp::RenderSelectedReconstruction() {
  if (reconstruction_manager_.Size() == 0) {
    RenderClear();
    return;
  }
...
void QgisApp::RenderOptions() {
  render_options_widget_->show();
  render_options_widget_->raise();
}

void QgisApp::SelectReconstructionIdx(const size_t) {
  RenderSelectedReconstruction();
}

size_t QgisApp::SelectedReconstructionIdx() {
...
bool QgisApp::HasSelectedReconstruction() {
...
bool QgisApp::IsSelectedReconstructionValid() {
  if (!HasSelectedReconstruction()) {
...
void QgisApp::ReconstructionStats() {
  if (!IsSelectedReconstructionValid()) {
    return;
  }
...
void QgisApp::MatchMatrix() { match_matrix_widget_->Show(); }

void QgisApp::ShowLog() {
  log_widget_->show();
  log_widget_->raise();
...
void QgisApp::ShowInvalidProjectError() {
  QMessageBox::critical(this, "",
    tr("You must create a valid project using: <i>File > "
      "New project</i> or <i>File > Edit project</i>"));
}

void QgisApp::EnableBlockingActions() {
  for (auto& action : blocking_actions_) {
    action->setEnabled(true);
  }
}

void QgisApp::DisableBlockingActions() {
  for (auto& action : blocking_actions_) {
    action->setDisabled(true);
  }
}

void QgisApp::UpdateWindowTitle() {
